<?php
	if ( isset($_POST['database'], $_POST['table'])) {
		...
		// DO TIMESTAMP
		$ts = date('[Y m d G:i:s]');
		$updateTimestamp = mysqli_connect(DB_HOST, DB_USER, DB_PASS);
//		$sql_update = "UPDATE ".$database.".".$table." SET `portrait`='portraits/user.png';";
		$sql_update = "UPDATE ".$database.".".$table." SET `time_start`='" . $ts . "';";
		$updateTimestamp->query($sql_update);
		mysqli_close($updateTimestamp);

		
		
		echo "\n\t\t\t<p>Showing table records of: <u>" . $database . "." . $table . "</u><br></p>";
